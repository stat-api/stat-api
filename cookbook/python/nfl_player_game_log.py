...
games = list(api.nfl.games.iter(season_id=season))

# Index games by id
game_by_id = {row["id"]: row for row in games}

# Borrow a player from one game's box score
seed = api.nfl.game_player_stats.list(game_id=games[0]["id"], limit=1).rows

# Page that player's game log
gamelog = list(api.nfl.game_player_stats.iter(player_id=seed[0]["player_id"]))

# Print the game log
print("NFL game log (fantasy points)")
print("\t".join(["game_time", "fantasy_pts"]))
for row in gamelog:
    print("\t".join([_cell((game_by_id.get(row["game_id"]) or {}).get("game_time", row["game_id"])), _cell(row["fantasy_pts"])]))
