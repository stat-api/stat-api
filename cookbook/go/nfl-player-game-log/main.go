...
func main() {
	...
	var games []statapi.NFLGame
	for row, err := range client.NFL.Games.All(ctx, &statapi.NFLGamesListParams{SeasonID: statapi.Int64(int64(season))}) {
		if err != nil {
			log.Fatal(err)
		}
	...
	// Index games by id
	game_by_id := map[int64]statapi.NFLGame{}
	for _, row := range games {
		game_by_id[row.ID] = row
	}

	// Borrow a player from one game's box score
	seedPage, err := client.NFL.GamePlayerStats.List(ctx, &statapi.NFLGamePlayerStatsListParams{GameID: statapi.Int64(int64(games[0].ID)), Limit: statapi.Int(1)})
	if err != nil {
		log.Fatal(err)
	}
	seed := seedPage.Rows
	...
	var gamelog []statapi.NFLGamePlayerStat
	for row, err := range client.NFL.GamePlayerStats.All(ctx, &statapi.NFLGamePlayerStatsListParams{PlayerID: statapi.Int64(int64(seed[0].PlayerID))}) {
		if err != nil {
			log.Fatal(err)
		}
	...
	// Print the game log
	fmt.Println("NFL game log (fantasy points)")
	fmt.Println(strings.Join([]string{"game_time", "fantasy_pts"}, "\t"))
