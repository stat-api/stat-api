...
func main() {
	...
	statsPage, err := client.NBA.GamePlayerStats.List(ctx, &statapi.NBAGamePlayerStatsListParams{GameID: statapi.Int64(int64(games[0].ID))})
	if err != nil {
		log.Fatal(err)
	}
	...
	by_team := map[int64][]statapi.NBAGamePlayerStat{}
