...
func main() {
	...
	var rows []statapi.NBASeasonTeamStat
	for row, err := range client.NBA.SeasonTeamStats.All(ctx, &statapi.NBASeasonTeamStatsListParams{SeasonID: statapi.Int64(int64(season))}) {
		if err != nil {
			log.Fatal(err)
		}
	...
	standings := append([]statapi.NBASeasonTeamStat(nil), rows...)
	standingsKey := func(r statapi.NBASeasonTeamStat) int {
		return r.Wins
	}
	...
	// Print the standings
	fmt.Println("NBA standings — top 10 by wins")
