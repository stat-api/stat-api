...
func main() {
	...
	var rows []statapi.MLBSeasonPlayerStat
	for row, err := range client.MLB.SeasonPlayerStats.All(ctx, &statapi.MLBSeasonPlayerStatsListParams{SeasonID: statapi.Int64(int64(season))}) {
		if err != nil {
			log.Fatal(err)
		}
	...
	// Rank by home runs
	ranked := append([]statapi.MLBSeasonPlayerStat(nil), rows...)
	rankedKey := func(r statapi.MLBSeasonPlayerStat) int {
		return r.HomeRuns
	}
	...
	fmt.Println("MLB home runs leaders")
	fmt.Println(strings.Join([]string{"player_id", "home_runs"}, "\t"))
	for _, row := range leaders {
		fmt.Println(strings.Join([]string{str(row.PlayerID), str(row.HomeRuns)}, "\t"))
	}
