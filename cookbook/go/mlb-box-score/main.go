...
func main() {
	...
	// Find a game
	gamesPage, err := client.MLB.Games.List(ctx, &statapi.MLBGamesListParams{SeasonID: statapi.Int64(int64(season)), Limit: statapi.Int(1)})
	if err != nil {
		log.Fatal(err)
	}
	...
	statsPage, err := client.MLB.GamePlayerBatterStats.List(ctx, &statapi.MLBGamePlayerBatterStatsListParams{GameID: statapi.Int64(int64(games[0].ID))})
	if err != nil {
		log.Fatal(err)
	}
	...
	by_team := map[int64][]statapi.MLBGamePlayerBatterStat{}
