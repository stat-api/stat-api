...
func main() {
	...
	var rows []statapi.NFLSeasonTeamStat
	for row, err := range client.NFL.SeasonTeamStats.All(ctx, &statapi.NFLSeasonTeamStatsListParams{SeasonID: statapi.Int64(int64(season))}) {
		if err != nil {
			log.Fatal(err)
		}
	...
	standings := append([]statapi.NFLSeasonTeamStat(nil), rows...)
	standingsKey := func(r statapi.NFLSeasonTeamStat) int {
		return r.Wins
	}
	...
	// Print the standings
	fmt.Println("NFL standings — top 10 by wins")
