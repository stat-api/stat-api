...
func main() {
	...
	// Find a game
	gamesPage, err := client.NHL.Games.List(ctx, &statapi.NHLGamesListParams{SeasonID: statapi.Int64(int64(season)), Limit: statapi.Int(1)})
	if err != nil {
		log.Fatal(err)
	}
	...
	statsPage, err := client.NHL.GamePlayerStats.List(ctx, &statapi.NHLGamePlayerStatsListParams{GameID: statapi.Int64(int64(games[0].ID))})
	if err != nil {
		log.Fatal(err)
	}
	...
	by_team := map[int64][]statapi.NHLGamePlayerStat{}
