...
func main() {
	...
	// Grab one team
	teamsPage, err := client.NFL.Teams.List(ctx, &statapi.NFLTeamsListParams{Limit: statapi.Int(1)})
	if err != nil {
		log.Fatal(err)
	}
	...
	rosterPage, err := client.NFL.Players.List(ctx, &statapi.NFLPlayersListParams{TeamID: statapi.Int64(int64(teams[0].ID))})
	if err != nil {
		log.Fatal(err)
	}
