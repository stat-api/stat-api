...
import { StatApi, type NFLGamePlayerStat } from '@stat-api/client'
...
// Find a game
const games = (await api.nfl.games.list({ season_id: season, limit: 1 })).games

// Pull per-player stats for that game
const stats = (await api.nfl.game_player_stats.list({ game_id: games[0].id })).game_player_stats

// Split the stat lines into the two teams
const by_team = new Map<number, NFLGamePlayerStat[]>()
