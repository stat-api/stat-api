...
// Find a game
var games = (await client.Nhl.Games.ListAsync(new NhlGamesListParams { SeasonId = season, Limit = 1 })).Rows;

// Pull per-player stats for that game
var stats = (await client.Nhl.GamePlayerStats.ListAsync(new NhlGamePlayerStatsListParams { GameId = games[0].Id })).Rows;
